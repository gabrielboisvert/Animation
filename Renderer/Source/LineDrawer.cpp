    ...
    : mCamera(pCamera), mRenderer(pRenderer), mUniBuffer(mRenderer)
{
    ...
    mUniBuffer.init(sizeof(Uniform), VK_SHADER_STAGE_VERTEX_BIT);
...
void LineDrawer::flushLines()
{
    ...
    mUniform.mVp = mCamera.mVp;

    vkCmdBindPipeline(mRenderer.mCommandBuffers[mRenderer.mCurrentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, mGraphicsPipeline);
    setVP(mUniBuffer.mDescriptorSets[mRenderer.mCurrentFrame], mUniBuffer.mUniformBuffersMapped[mRenderer.mCurrentFrame], &mUniform, sizeof(Uniform));
